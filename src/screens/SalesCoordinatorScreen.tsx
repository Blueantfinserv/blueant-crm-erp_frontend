...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
    ...
    {!loading && !error && tab === 'tasks' ? <View style={[styles.taskDashboard, compact && styles.taskDashboardCompact]}>
      ...
      <View style={styles.taskListPanel}><View style={[styles.filters, compact && styles.filtersCompact]}><TextInput value={taskSearch} onChangeText={setTaskSearch} placeholder="Search lead name or number" placeholderTextColor="#94A3B8" style={styles.filterInput} /><View style={styles.chips}>{(['ALL', 'PENDING', 'OVERDUE'] as const).map((s) => <Pressable key={s} onPress={() => setStatusFilter(s)} style={[styles.chip, statusFilter === s && styles.chipActive]}><Text style={[styles.chipText, statusFilter === s && styles.chipTextActive]}>{s === 'ALL' ? 'ALL TASKS' : `${s} TASKS`}</Text></Pressable>)}</View></View>
        <View style={styles.taskList}>{!compact ? <MeetingTableHeader taskOnly records={taskMeetings} filters={taskColumnFilters} onFiltersChange={setTaskColumnFilters} /> : null}<Cards taskOnly items={filteredTasks} empty="No meeting tasks match these client-side filters." /></View>
      </View>
    </View> : null}
    </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  taskSummaryPanelCompact: { width: '100%' },
