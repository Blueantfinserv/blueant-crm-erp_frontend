...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  const [responseView, setResponseView] = useState<'verified' | 'cancelled'>('verified');
  ...
  const cancelledMeetings = useMemo(() => meetings.filter((meeting) => String(meeting.meetingStatus ?? '').toUpperCase() === 'CANCELLED'), [meetings]);
  const visibleCancelledMeetings = useMemo(() => cancelledMeetings.filter((meeting) => matchesSearch(meeting.clientName, responseSearch) || matchesSearch(meeting.mobileNumber, responseSearch)), [cancelledMeetings, responseSearch]);
  ...
  const cancelSelectedMeeting = async () => {
    if (!selected?.meetingCode || selected.id === undefined || submittingRef.current) return;
    submittingRef.current = true; setSubmitting(true); setSubmitError(null);
    try {
      await meetingService.cancelMeeting(selected.meetingCode, { meetingId: selected.id, cancellationReason: 'Cancelled by Sales Coordinator' });
      const cancelled = { ...selected, meetingStatus: 'CANCELLED' as const };
      setPending((items) => items.filter((meeting) => meeting.meetingCode !== selected.meetingCode));
      setMeetings((items) => [cancelled, ...items.filter((meeting) => meeting.meetingCode !== selected.meetingCode)]);
      setSelected(null);
    } catch (e) { setSubmitError(e instanceof Error ? e.message : 'Meeting cancellation failed.'); }
    finally { submittingRef.current = false; setSubmitting(false); }
  };
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
      ...
      <View style={styles.contentHeading}>
        <View style={[styles.contentHeadingRow, compact && styles.contentHeadingRowCompact]}>
          <View>
            ...
            <Text style={styles.contentSubtitle}>{tab === 'today' ? `Showing ${visibleTodayMeetings.length} of ${pending.length} meetings` : tab === 'responses' ? `Showing ${visibleVerifiedMeetings.length} of ${verified.length} verified meetings` : tab === 'tasks' ? `Showing ${filteredTasks.length} of ${meetings.length} meeting tasks` : tab === 'assignedLeads' ? `Showing ${visibleAssignedLeads.length} of ${assignedLeads.length} assigned leads` : 'Create a physical lead and assign it to a sales person'}</Text>
          </View>
    ...
    {!loading && !error && tab === 'responses' ? <View style={styles.responseContent}><View style={styles.responseTabs}><Pressable onPress={() => setResponseView('verified')} style={[styles.responseTab, responseView === 'verified' && styles.responseTabActive]}><Text style={[styles.responseTabText, responseView === 'verified' && styles.responseTabTextActive]}>Verified ({visibleVerifiedMeetings.length})</Text></Pressable><Pressable onPress={() => setResponseView('cancelled')} style={[styles.responseTab, styles.cancelledResponseTab, responseView === 'cancelled' && styles.cancelledResponseTabActive]}><Text style={[styles.responseTabText, styles.cancelledResponseTabText, responseView === 'cancelled' && styles.responseTabTextActive]}>Cancelled ({visibleCancelledMeetings.length})</Text></Pressable></View><Cards items={responseView === 'verified' ? visibleVerifiedMeetings : visibleCancelledMeetings} empty={responseView === 'verified' ? 'No meetings have been verified by the Sales Coordinator yet.' : 'No meetings have been cancelled by the Sales Coordinator yet.'} action="View Response" onOpen={setSelected} verified /></View> : null}
    ...
    <Modal transparent visible={Boolean(selected)} animationType="fade" onRequestClose={() => setSelected(null)}><View style={styles.backdrop}><View style={styles.modal}>
      <View style={styles.modalHeader}><View style={styles.modalHeaderCopy}><View style={styles.modalEyebrowRow}><View style={styles.modalEyebrowDot} /><Text style={styles.modalEyebrow}>{tab === 'responses' ? 'VERIFIED RESPONSE' : 'PENDING VERIFICATION'}</Text></View><Text numberOfLines={1} style={styles.modalTitle}>{selected?.clientName ?? selected?.meetingCode}</Text></View>{tab === 'today' ? <Pressable disabled={submitting || selected?.id === undefined} onPress={() => void cancelSelectedMeeting()} style={[styles.cancelMeetingButton, submitting && styles.disabled]}><Icon source="cancel" size={16} color="#FFFFFF" /><Text style={styles.cancelMeetingButtonText}>Cancel</Text></Pressable> : null}<Pressable onPress={() => setSelected(null)} style={styles.close}><Icon source="close" size={22} color="#334155" /></Pressable></View>
...
const styles = StyleSheet.create({
  headerSearch: { width: 155, height: 32, paddingHorizontal: 10, borderWidth: 1, borderColor: '#91A7E0', borderRadius: 7, color: '#FFFFFF', fontSize: 9, fontWeight: '700', backgroundColor: 'rgba(255,255,255,0.12)' }, headerSearchCompact: { width: '100%', height: 38, fontSize: 11 },
  ...
  responseTabs: { flexDirection: 'row', gap: 7, paddingHorizontal: 13, paddingTop: 9 },
  responseTab: { paddingHorizontal: 11, paddingVertical: 6, borderWidth: 1, borderColor: '#D8D2FE', borderRadius: 99, backgroundColor: '#F5F3FF' },
  responseTabActive: { borderColor: '#7C3AED', backgroundColor: '#7C3AED' },
  responseTabText: { color: '#6D28D9', fontSize: 8, fontWeight: '900' },
  responseTabTextActive: { color: '#FFFFFF' },
  cancelledResponseTab: { borderColor: '#FED7AA', backgroundColor: '#FFF7ED' },
  cancelledResponseTabActive: { borderColor: '#EA580C', backgroundColor: '#EA580C' },
  cancelledResponseTabText: { color: '#C2410C' },
