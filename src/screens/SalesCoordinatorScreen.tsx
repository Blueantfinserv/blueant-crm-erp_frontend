...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
    ...
    {!compact && !loading && !error && tab !== 'assign' && tab !== 'assignedLeads' ? <View style={tab === 'tasks' ? styles.taskStickyHeader : undefined}><MeetingTableHeader verified={tab === 'responses'} taskOnly={tab === 'tasks'} records={tab === 'tasks' ? taskMeetings : tab === 'responses' ? verified : pending} filters={tab === 'tasks' ? taskColumnFilters : tab === 'responses' ? responseColumnFilters : todayColumnFilters} onFiltersChange={tab === 'tasks' ? setTaskColumnFilters : tab === 'responses' ? setResponseColumnFilters : setTodayColumnFilters} /></View> : null}
    {tab === 'tasks' ? <ScrollView style={styles.resultsScroll} contentContainerStyle={styles.resultsContent} showsVerticalScrollIndicator><View style={styles.taskFullPanel}>{permissions?.length ? <Text style={styles.permission}>API access uses the permission codes returned in this authenticated session.</Text> : null}{loading ? <State loading message="Loading coordinator workspace..." /> : error ? <State message={error} /> : null}{!loading && !error ? <View style={styles.taskList}><Cards taskOnly items={filteredTasks} empty="No meeting tasks match these filters." /></View> : null}</View></ScrollView> : null}
...
const styles = StyleSheet.create({
  ...
  taskFullPanel: { gap: 10, padding: 10 }, taskStickyHeader: { marginLeft: 15, marginRight: 38, overflow: 'hidden', borderTopLeftRadius: 12, borderTopRightRadius: 12, backgroundColor: '#EEF3FF' }, taskToolbar: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 7, padding: 10, borderWidth: 1, borderColor: '#DCE5FA', borderRadius: 10, backgroundColor: '#F8FAFF' }, taskToolbarHeader: { minWidth: 0, flex: 1, justifyContent: 'flex-end', padding: 0, borderWidth: 0, borderRadius: 0, backgroundColor: 'transparent' }, taskSalesPicker: { width: 190, height: 34, flexDirection: 'row', alignItems: 'center', gap: 3, paddingLeft: 8, overflow: 'hidden', borderWidth: 1, borderColor: '#D4E0FF', borderRadius: 10, backgroundColor: '#F9FBFF' }, taskSalesPickerControl: { minWidth: 0, flex: 1, height: 34, color: '#1E3A8A', fontSize: 9, fontWeight: '900' }, taskSummaryPanelCompact: { width: '100%' }, taskResultsContent: { flex: 1, minHeight: 0 }, taskDashboardFill: { flex: 1, minHeight: 0 }, taskListFill: { flex: 1, minHeight: 0 }, taskRowsScroll: { flex: 1, minHeight: 0 }, taskRowsContent: { flexGrow: 0 },
