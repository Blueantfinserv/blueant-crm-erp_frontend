...
const exportMeetingCsv = (items: readonly MeetingResponse[], fileName: string, includeVerifier: boolean) => {
  ...
  const headers = ['Client Name', 'Mobile Number', 'Meeting Type', 'Sales Person', 'Lead Status', 'Meeting Date', 'Next Plan Date', 'Joined'];
  if (includeVerifier) headers.push('Verified By', 'Verification Date');
  const rows = items.map((meeting) => [meeting.clientName, meeting.mobileNumber, meeting.meetingTitle ?? meeting.meetingType, meeting.employeeName, meeting.leadStatus?.replace(/_/g, ' '), meeting.meetingDate, meeting.nextMeetingDate, meeting.aloneWith, ...(includeVerifier ? [meeting.verifiedBy, meeting.meetingVerificationDate] : [])]);
...
function MeetingTableHeader({ verified = false, records, filters, onFiltersChange }: { verified?: boolean; records: readonly MeetingResponse[]; filters: MeetingColumnFilter; onFiltersChange: React.Dispatch<React.SetStateAction<MeetingColumnFilter>> }) {
  ...
  const dropdownKeys: (keyof MeetingColumnFilter)[] = ['employeeName', 'verifiedBy', 'meetingTitle', 'leadStatus'];
...
const styles = StyleSheet.create({
  locationSection: { gap: 8, marginTop: 18, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#E2E8F0' },
  ...
  clearFiltersText: { color: '#3156C8', fontSize: 8, fontWeight: '800' },
