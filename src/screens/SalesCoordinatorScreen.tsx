...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  const openLeadHistory = async (lead: LeadResponse) => {
    ...
    try {
      ...
      [...localMeetings, ...summaries].forEach((meeting) => { const key = meeting.meetingCode ?? String(meeting.id ?? ''); if (key) byCode.set(key, { ...byCode.get(key), ...meeting }); });
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
    ...
    <Modal transparent visible={Boolean(historyLead)} animationType="fade" onRequestClose={() => setHistoryLead(null)}><View style={styles.backdrop}><View style={styles.modal}>
      ...
      <ScrollView contentContainerStyle={styles.modalBody}>{historyLead ? <LeadHistoryContentCompact lead={historyLead} meetings={historyMeetings} loading={historyLoading} error={historyError} /> : null}</ScrollView>
    </View></View></Modal>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  detailsWrap: { gap: 11 }, historyWrap: { gap: 9 }, compactDetailGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, compactDetail: { width: '23%', minWidth: 130, flexGrow: 1, paddingHorizontal: 8, paddingVertical: 6 }, historyEmpty: { color: '#64748B', fontSize: 9, fontWeight: '700' }, historyTitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, historyCount: { color: '#64748B', fontSize: 9, fontWeight: '800' }, compactTimeline: { gap: 7 }, compactTimelineCard: { gap: 5, padding: 9, borderWidth: 1, borderColor: '#DCE5F3', borderRadius: 10, backgroundColor: '#F8FAFF' }, compactRemark: { color: '#334155', fontSize: 9, fontWeight: '600', lineHeight: 13 }, timeline: { gap: 0 }, timelineItem: { flexDirection: 'row', minHeight: 92 }, timelineRail: { width: 24, alignItems: 'center' }, timelineDot: { width: 11, height: 11, marginTop: 12, borderRadius: 6, backgroundColor: '#3156C8', borderWidth: 3, borderColor: '#DBEAFE' }, timelineLine: { width: 2, flex: 1, marginVertical: 3, backgroundColor: '#D6E1F7' }, timelineCard: { minWidth: 0, flex: 1, gap: 7, marginBottom: 10, padding: 11, borderWidth: 1, borderColor: '#DCE5F3', borderRadius: 12, backgroundColor: '#F8FAFF' }, timelineTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }, timelineTitle: { minWidth: 0, flex: 1, color: '#1E3A8A', fontSize: 11, fontWeight: '900' }, timelineStatus: { maxWidth: '46%', paddingHorizontal: 7, paddingVertical: 3, overflow: 'hidden', borderRadius: 99, color: '#A95A08', fontSize: 7, fontWeight: '900', backgroundColor: '#FFF7E8' }, timelineMeta: { color: '#64748B', fontSize: 8, fontWeight: '700' }, timelineValue: { color: '#334155', fontSize: 9, fontWeight: '600', lineHeight: 13 }, timelineRemark: { gap: 3, padding: 8, borderRadius: 8, backgroundColor: '#FFF8E8' }, timelineRemarkLabel: { color: '#A95A08', fontSize: 7, fontWeight: '900', letterSpacing: 0.45 }, timelineInfo: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, timelineInfoCell: { minWidth: 135, flex: 1, gap: 2, padding: 7, borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, backgroundColor: '#FFFFFF' }, timelineLocation: { gap: 3, padding: 8, borderWidth: 1, borderColor: '#BEE8DE', borderRadius: 8, backgroundColor: '#F0FDFA' }, historyMapButton: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 3, paddingHorizontal: 9, paddingVertical: 6, borderRadius: 7, backgroundColor: '#0F766E' }, historyMapButtonText: { color: '#FFFFFF', fontSize: 8, fontWeight: '900' },
