...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
    ...
    {!compact && !loading && !error && tab !== 'assign' && tab !== 'assignedLeads' ? <MeetingTableHeader verified={tab === 'responses'} records={tab === 'responses' ? verified : tab === 'tasks' ? taskMeetings : pending} filters={tab === 'responses' ? responseColumnFilters : tab === 'tasks' ? taskColumnFilters : todayColumnFilters} onFiltersChange={tab === 'responses' ? setResponseColumnFilters : tab === 'tasks' ? setTaskColumnFilters : setTodayColumnFilters} /> : null}
