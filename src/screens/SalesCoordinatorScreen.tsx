...
type AssignedDateFilter = 'ALL' | 'TODAY' | 'THIS_WEEK';
type TodayMeetingView = 'PENDING' | 'FLAGGED';
...
const matchesAssignedDate = (assignedAt: string | undefined, filter: AssignedDateFilter) => {
  ...
  return date >= calendarDate(weekStart) && date <= calendarDate(weekEnd);
};
...
const meetingKey = (meeting: MeetingResponse) => meeting.meetingCode ?? String(meeting.id ?? '');
...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  const [todayMeetingView, setTodayMeetingView] = useState<TodayMeetingView>('PENDING');
  const [flaggedMeetingCodes, setFlaggedMeetingCodes] = useState<string[]>([]);
  const [flagTarget, setFlagTarget] = useState<MeetingResponse | null>(null);
  const [unflagTarget, setUnflagTarget] = useState<MeetingResponse | null>(null);
  ...
  const [assignedDateFilter, setAssignedDateFilter] = useState<AssignedDateFilter>('ALL');
  ...
  const filteredTasks = useMemo(() => {
    ...
    const matchingTasks = taskMeetings.filter((m) => {
  ...
  }, [statusFilter, taskColumnFilters, taskMeetings, taskSearch]);
  const pendingTodayMeetings = useMemo(() => pending.filter((meeting) => !flaggedMeetingCodes.includes(meetingKey(meeting))), [flaggedMeetingCodes, pending]);
  const flaggedTodayMeetings = useMemo(() => pending.filter((meeting) => flaggedMeetingCodes.includes(meetingKey(meeting))), [flaggedMeetingCodes, pending]);
  const visibleTodayMeetings = useMemo(() => filterMeetingRecords(todayMeetingView === 'FLAGGED' ? flaggedTodayMeetings : pendingTodayMeetings, todayColumnFilters), [flaggedTodayMeetings, pendingTodayMeetings, todayColumnFilters, todayMeetingView]);
  ...
  const visibleAssignedLeads = useMemo(() => assignedLeads.filter((lead) => (
    ...
    && matchesAssignedDate(lead.assignedAt, assignedDateFilter)
  )), [assignedDateFilter, assignedLeads, assignedSalesPersonFilter, assignedSearch, coordinatorFilter]);
  const summaries = useMemo(() => taskMeetings.reduce<Record<string, { name: string; TODAY: number; PENDING: number; OVERDUE: number }>>((result, m) => {
  ...
  const taskSalesPeople = useMemo(() => [...new Set(taskMeetings.map((meeting) => meeting.employeeName).filter(Boolean))].sort() as string[], [taskMeetings]);
  ...
  const confirmFlagMeeting = () => {
    if (!flagTarget) return;
    const code = meetingKey(flagTarget);
    if (code) setFlaggedMeetingCodes((codes) => codes.includes(code) ? codes : [...codes, code]);
    setFlagTarget(null);
    setTodayMeetingView('FLAGGED');
  };
  const confirmUnflagMeeting = () => {
    if (!unflagTarget) return;
    const code = meetingKey(unflagTarget);
    setFlaggedMeetingCodes((codes) => codes.filter((flaggedCode) => flaggedCode !== code));
    setUnflagTarget(null);
    setTodayMeetingView('PENDING');
  };
  const verify = async () => {
  ...
  const metricsBlock = <View style={styles.metrics}><Metric icon="clock-outline" value={pendingTodayMeetings.length} label="To review" tone="blue" /><Metric icon="account-outline" value={Object.keys(summaries).length} label="Sales people" tone="orange" /></View>;
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
      ...
      <View style={styles.contentHeading}>
        <View style={[styles.contentHeadingRow, compact && styles.contentHeadingRowCompact]}>
          <View>
            ...
            <Text style={styles.contentSubtitle}>{tab === 'today' ? `Showing ${visibleTodayMeetings.length} ${todayMeetingView === 'FLAGGED' ? 'flagged' : 'pending'} meetings` : tab === 'responses' ? `Showing ${visibleVerifiedMeetings.length} of ${verified.length} verified meetings` : tab === 'tasks' ? `Showing ${filteredTasks.length} of ${taskMeetings.length} active tasks` : tab === 'assignedLeads' ? `Showing ${visibleAssignedLeads.length} of ${assignedLeads.length} assigned leads` : 'Create a physical lead and assign it to a sales person'}</Text>
          </View>
          ...
          {tab === 'assignedLeads' ? <HeaderLeadFilters leads={assignedLeads} coordinatorFilter={coordinatorFilter} setCoordinatorFilter={setCoordinatorFilter} salesPersonFilter={assignedSalesPersonFilter} setSalesPersonFilter={setAssignedSalesPersonFilter} dateFilter={assignedDateFilter} setDateFilter={setAssignedDateFilter} search={assignedSearch} setSearch={setAssignedSearch} /> : (tab === 'today' || tab === 'responses') ? <Pressable onPress={exportCurrentList} style={({ pressed }) => [styles.exportButton, compact && styles.exportButtonCompact, pressed && styles.pressed]}><Icon source="file-excel-outline" size={15} color="#FFFFFF" /><Text style={styles.exportButtonText}>Export Excel</Text></Pressable> : null}
        </View>
        {tab === 'today' ? <View style={styles.todaySubtabs}><Pressable onPress={() => setTodayMeetingView('PENDING')} style={[styles.todaySubtab, todayMeetingView === 'PENDING' && styles.todaySubtabActive]}><Text style={[styles.todaySubtabText, todayMeetingView === 'PENDING' && styles.todaySubtabTextActive]}>To Review ({pendingTodayMeetings.length})</Text></Pressable><Pressable onPress={() => setTodayMeetingView('FLAGGED')} style={[styles.todaySubtab, todayMeetingView === 'FLAGGED' && styles.flaggedSubtabActive]}><Icon source="flag" size={13} color={todayMeetingView === 'FLAGGED' ? '#FFFFFF' : '#DC2626'} /><Text style={[styles.todaySubtabText, styles.flaggedSubtabText, todayMeetingView === 'FLAGGED' && styles.todaySubtabTextActive]}>Flagged ({flaggedTodayMeetings.length})</Text></Pressable></View> : null}
      </View>
    {!compact && !loading && !error && tab !== 'assign' && tab !== 'tasks' && tab !== 'assignedLeads' ? <MeetingTableHeader verified={tab === 'responses'} records={tab === 'responses' ? verified : (todayMeetingView === 'FLAGGED' ? flaggedTodayMeetings : pendingTodayMeetings)} filters={tab === 'responses' ? responseColumnFilters : todayColumnFilters} onFiltersChange={tab === 'responses' ? setResponseColumnFilters : setTodayColumnFilters} /> : null}
    {tab === 'tasks' ? <><View style={styles.taskToolbarWrap}>{!loading && !error ? <TaskToolbar search={taskSearch} onSearch={setTaskSearch} status={statusFilter} onStatus={setStatusFilter} salesPeople={taskSalesPeople} selectedSalesPerson={taskColumnFilters.employeeName ?? ''} onSalesPersonChange={(employeeName) => setTaskColumnFilters((current) => ({ ...current, employeeName }))} /> : null}</View><ScrollView style={styles.resultsScroll} contentContainerStyle={styles.resultsContent} showsVerticalScrollIndicator><View style={styles.taskFullPanel}>{permissions?.length ? <Text style={styles.permission}>API access uses the permission codes returned in this authenticated session.</Text> : null}{loading ? <State loading message="Loading coordinator workspace..." /> : error ? <State message={error} /> : null}{!loading && !error ? <View style={styles.taskList}>{!compact ? <MeetingTableHeader taskOnly records={taskMeetings} filters={taskColumnFilters} onFiltersChange={setTaskColumnFilters} /> : null}<Cards taskOnly items={filteredTasks} empty="No meeting tasks match these filters." /></View> : null}</View></ScrollView></> : null}
    {tab !== 'tasks' ? <ScrollView style={styles.resultsScroll} contentContainerStyle={styles.resultsContent} showsVerticalScrollIndicator={false}>
    ...
    {!loading && !error && tab === 'today' ? <Cards items={visibleTodayMeetings} empty={todayMeetingView === 'FLAGGED' ? 'No meetings have been flagged.' : 'No meetings are pending Process Coordinator verification.'} action="Verify Details" onOpen={openVerify} onFlag={todayMeetingView === 'PENDING' ? setFlagTarget : undefined} onUnflag={todayMeetingView === 'FLAGGED' ? setUnflagTarget : undefined} /> : null}
    ...
    <Modal transparent visible={Boolean(flagTarget)} animationType="fade" onRequestClose={() => setFlagTarget(null)}><Pressable style={styles.backdrop} onPress={() => setFlagTarget(null)}><Pressable style={styles.flagModal} onPress={() => undefined}><View style={styles.flagModalIcon}><Icon source="flag" size={24} color="#DC2626" /></View><Text style={styles.flagModalTitle}>Flag this meeting?</Text><Text style={styles.flagModalText}>{flagTarget?.clientName ?? 'This meeting'} will move from To Review to the Flagged tab.</Text><View style={styles.flagModalActions}><Pressable onPress={() => setFlagTarget(null)} style={styles.flagCancel}><Text style={styles.flagCancelText}>Cancel</Text></Pressable><Pressable onPress={confirmFlagMeeting} style={styles.flagConfirm}><Icon source="flag" size={14} color="#FFFFFF" /><Text style={styles.flagConfirmText}>Yes, Flag</Text></Pressable></View></Pressable></Pressable></Modal>
    <Modal transparent visible={Boolean(unflagTarget)} animationType="fade" onRequestClose={() => setUnflagTarget(null)}><Pressable style={styles.backdrop} onPress={() => setUnflagTarget(null)}><Pressable style={styles.flagModal} onPress={() => undefined}><View style={styles.unflagModalIcon}><Icon source="flag-outline" size={24} color="#B45309" /></View><Text style={styles.flagModalTitle}>Unflag this meeting?</Text><Text style={styles.flagModalText}>{unflagTarget?.clientName ?? 'This meeting'} will return to the To Review tab.</Text><View style={styles.flagModalActions}><Pressable onPress={() => setUnflagTarget(null)} style={styles.flagCancel}><Text style={styles.flagCancelText}>Cancel</Text></Pressable><Pressable onPress={confirmUnflagMeeting} style={styles.unflagConfirm}><Icon source="flag-outline" size={14} color="#FFFFFF" /><Text style={styles.flagConfirmText}>Yes, Unflag</Text></Pressable></View></Pressable></Pressable></Modal>
  </View>;
}
...
function Cards({ items, filters = {}, empty, action, onOpen, onFlag, onUnflag, verified = false, taskOnly = false }: { items: readonly MeetingResponse[]; filters?: MeetingColumnFilter; empty: string; action?: string; onOpen?: (m: MeetingResponse) => void; onFlag?: (m: MeetingResponse) => void; onUnflag?: (m: MeetingResponse) => void; verified?: boolean; taskOnly?: boolean }) {
...
function LeadHistoryContentEnhanced({ lead, meetings, loading, error }: { lead: LeadResponse; meetings: MeetingResponse[]; loading: boolean; error: string | null }) {
  ...
  const details = [['Mobile number', lead.mobileNumber], ['Location', lead.location], ['Clinic address', lead.clinicAddress], ['Speciality', lead.speciality], ['Lead code', lead.leadCode], ['Latest remarks', latest?.remarks ?? latest?.meetingRemarks ?? latest?.discussion ?? lead.remarks]];
...
function LeadHistoryContentCompact({ lead, meetings, loading, error }: { lead: LeadResponse; meetings: MeetingResponse[]; loading: boolean; error: string | null }) {
  ...
  const leadFields = [['Mobile number', lead.mobileNumber], ['Location', lead.location], ['Clinic address', lead.clinicAddress], ['Speciality', lead.speciality], ['Lead code', lead.leadCode], ['Latest remarks', latestRemark?.remarks ?? latestRemark?.meetingRemarks ?? latestRemark?.discussion ?? lead.remarks]];
...
function LeadHistoryContentPretty({ lead, meetings, loading, error }: { lead: LeadResponse; meetings: MeetingResponse[]; loading: boolean; error: string | null }) {
  ...
  const leadFields = [['Mobile number', lead.mobileNumber], ['Location', lead.location], ['Clinic address', lead.clinicAddress], ['Speciality', lead.speciality], ['Lead code', lead.leadCode]];
  const assignmentFields = [['Sales person', lead.assignedEmployeeName], ['Sales person ID', lead.assignedEmployeeCode], ['Sales coordinator', lead.assignedByEmployeeName], ['SC ID', lead.assignedByEmployeeCode], ['Assigned date', lead.assignedAt], ['Current status', lead.leadStatus?.replace(/_/g, ' ')]];
...
const styles = StyleSheet.create({
  ...
  taskDashboard: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, padding: 10 }, taskDashboardCompact: { flexDirection: 'column' }, taskSummaryPanel: { width: 310, flexShrink: 0 }, taskListPanel: { minWidth: 0, flex: 1, gap: 8 }, summaries: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, summary: { width: '48.8%', minWidth: 0, flexGrow: 0, paddingHorizontal: 9, paddingVertical: 7, borderWidth: 1, borderColor: '#DDE7FF', borderRadius: 9, backgroundColor: '#F7F9FF' }, summaryCompact: { width: '48%', minWidth: 0, flexGrow: 0 }, summaryName: { color: '#1E3A8A', fontSize: 12, fontWeight: '900' }, summaryCounts: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 3 }, summaryLine: { color: '#64748B', fontSize: 8, fontWeight: '700' }, summaryCount: { color: '#1E3A8A', fontWeight: '900' }, overdueCount: { color: '#C2410C' }, filters: { flexDirection: 'row', alignItems: 'center', gap: 8 }, filtersCompact: { flexDirection: 'column', alignItems: 'stretch' }, filterInput: { minWidth: 230, height: 36, paddingHorizontal: 12, borderWidth: 1, borderColor: '#D7DDE8', borderRadius: 9, backgroundColor: '#FFFFFF', color: '#172033', fontSize: 10, fontWeight: '700' }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, chip: { paddingHorizontal: 11, paddingVertical: 9, borderRadius: 9, backgroundColor: '#E9EDF5' }, chipActive: { backgroundColor: '#3156C8' }, chipText: { color: '#536078', fontSize: 8, fontWeight: '900' }, chipTextActive: { color: '#FFFFFF' }, taskList: { gap: 0, overflow: 'hidden', borderWidth: 1, borderColor: '#DCE5FA', borderRadius: 10 },
  taskFullPanel: { gap: 10, padding: 10 }, taskToolbar: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8, padding: 10, borderWidth: 1, borderColor: '#DCE5FA', borderRadius: 10, backgroundColor: '#F8FAFF' }, taskSalesPicker: { width: 190, height: 36, flexDirection: 'row', alignItems: 'center', gap: 3, paddingLeft: 8, overflow: 'hidden', borderWidth: 1, borderColor: '#C7D2FE', borderRadius: 9, backgroundColor: '#FFFFFF' }, taskSalesPickerControl: { minWidth: 0, flex: 1, height: 36, color: '#1E3A8A', fontSize: 10, fontWeight: '800' }, taskSummaryPanelCompact: { width: '100%' }, taskResultsContent: { flex: 1, minHeight: 0 }, taskDashboardFill: { flex: 1, minHeight: 0 }, taskListFill: { flex: 1, minHeight: 0 }, taskRowsScroll: { flex: 1, minHeight: 0 }, taskRowsContent: { flexGrow: 0 },
  ...
  todaySubtabs: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 10 }, todaySubtab: { minHeight: 30, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 11, borderWidth: 1, borderColor: '#C7D2FE', borderRadius: 8, backgroundColor: '#FFFFFF' }, todaySubtabActive: { borderColor: '#5269DE', backgroundColor: '#5269DE' }, flaggedSubtabActive: { borderColor: '#DC2626', backgroundColor: '#DC2626' }, todaySubtabText: { color: '#3156C8', fontSize: 9, fontWeight: '900' }, flaggedSubtabText: { color: '#DC2626' }, todaySubtabTextActive: { color: '#FFFFFF' }, rowActions: { flexDirection: 'row', alignItems: 'center', gap: 6 }, rowActionInline: { width: undefined, minWidth: 0, flex: 1 }, flagButton: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#FECACA', borderRadius: 15, backgroundColor: '#FEF2F2' }, unflagButton: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#FDE68A', borderRadius: 15, backgroundColor: '#FFFBEB' }, flagModal: { width: '100%', maxWidth: 350, alignItems: 'center', padding: 22, borderRadius: 18, backgroundColor: '#FFFFFF' }, flagModalIcon: { width: 50, height: 50, alignItems: 'center', justifyContent: 'center', borderRadius: 25, backgroundColor: '#FEF2F2' }, unflagModalIcon: { width: 50, height: 50, alignItems: 'center', justifyContent: 'center', borderRadius: 25, backgroundColor: '#FFFBEB' }, flagModalTitle: { marginTop: 12, color: '#1E293B', fontSize: 18, fontWeight: '900' }, flagModalText: { marginTop: 7, color: '#64748B', fontSize: 11, fontWeight: '600', lineHeight: 17, textAlign: 'center' }, flagModalActions: { width: '100%', flexDirection: 'row', gap: 8, marginTop: 18 }, flagCancel: { flex: 1, alignItems: 'center', justifyContent: 'center', minHeight: 38, borderWidth: 1, borderColor: '#D8E0EB', borderRadius: 9 }, flagCancelText: { color: '#475569', fontSize: 10, fontWeight: '900' }, flagConfirm: { flex: 1, minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, borderRadius: 9, backgroundColor: '#DC2626' }, unflagConfirm: { flex: 1, minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, borderRadius: 9, backgroundColor: '#B45309' }, flagConfirmText: { color: '#FFFFFF', fontSize: 10, fontWeight: '900' },
});
