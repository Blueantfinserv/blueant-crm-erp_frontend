...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  return <View style={[styles.page, compact && styles.pageCompact]}>
    ...
    {!loading && !error && tab === 'assignedLeads' ? <AssignedLeadCards leads={assignedLeads} coordinatorFilter={coordinatorFilter} setCoordinatorFilter={setCoordinatorFilter} salesPersonFilter={assignedSalesPersonFilter} setSalesPersonFilter={setAssignedSalesPersonFilter} /> : null}
...
function HeaderLeadFilters({ leads, coordinatorFilter, setCoordinatorFilter, salesPersonFilter, setSalesPersonFilter }: { leads: LeadResponse[]; coordinatorFilter: string; setCoordinatorFilter: (value: string) => void; salesPersonFilter: string; setSalesPersonFilter: (value: string) => void }) { const coordinators = [...new Set(leads.map((lead) => lead.assignedByEmployeeName).filter(Boolean))] as string[]; const salesPeople = [...new Set(leads.filter((lead) => !coordinatorFilter || lead.assignedByEmployeeName === coordinatorFilter).map((lead) => lead.assignedEmployeeName).filter(Boolean))] as string[]; return <View style={styles.headerLeadFilters}><View style={styles.headerLeadFilter}><Icon source="account-tie-outline" size={15} color="#BFD1FF" /><View style={styles.headerLeadPicker}><Picker selectedValue={coordinatorFilter} onValueChange={(value) => { setCoordinatorFilter(String(value)); setSalesPersonFilter(''); }} style={styles.headerLeadPickerControl}><Picker.Item label="All Coordinators" value="" />{coordinators.map((name) => <Picker.Item key={name} label={name} value={name} />)}</Picker></View></View><View style={[styles.headerLeadFilter, !coordinatorFilter && styles.headerLeadPickerDisabled]}><Icon source="account-outline" size={15} color="#BFD1FF" /><View style={styles.headerLeadPicker}><Picker enabled={Boolean(coordinatorFilter)} selectedValue={salesPersonFilter} onValueChange={(value) => setSalesPersonFilter(String(value))} style={styles.headerLeadPickerControl}><Picker.Item label={coordinatorFilter ? 'All Sales Persons' : 'Select coordinator first'} value="" />{salesPeople.map((name) => <Picker.Item key={name} label={name} value={name} />)}</Picker></View></View></View>; }
function AssignedLeadCards({ leads, coordinatorFilter, setCoordinatorFilter, salesPersonFilter, setSalesPersonFilter }: { leads: LeadResponse[]; coordinatorFilter: string; setCoordinatorFilter: (value: string) => void; salesPersonFilter: string; setSalesPersonFilter: (value: string) => void }) { const coordinators = [...new Set(leads.map((lead) => lead.assignedByEmployeeName).filter(Boolean))] as string[]; const salesPeople = [...new Set(leads.filter((lead) => !coordinatorFilter || lead.assignedByEmployeeName === coordinatorFilter).map((lead) => lead.assignedEmployeeName).filter(Boolean))] as string[]; const visible = leads.filter((lead) => (!coordinatorFilter || lead.assignedByEmployeeName === coordinatorFilter) && (!salesPersonFilter || lead.assignedEmployeeName === salesPersonFilter)); return <View style={styles.assignedLeadWrap}><View style={styles.coordinatorFilterCard}><View style={styles.coordinatorFilterIcon}><Icon source="filter-variant" size={19} color="#3156C8" /></View><View style={styles.coordinatorFilterCopy}><Text style={styles.coordinatorFilterTitle}>Assigned lead filters</Text><Text style={styles.coordinatorFilterHint}>Filter by coordinator or sales person</Text></View><View style={styles.coordinatorPicker}><Picker selectedValue={coordinatorFilter} onValueChange={(value) => { setCoordinatorFilter(String(value)); setSalesPersonFilter(''); }} style={styles.assignPicker}><Picker.Item label="All Sales Coordinators" value="" />{coordinators.map((name) => <Picker.Item key={name} label={name} value={name} />)}</Picker></View><View style={[styles.coordinatorPicker, !coordinatorFilter && styles.coordinatorPickerDisabled]}><Picker enabled={Boolean(coordinatorFilter)} selectedValue={salesPersonFilter} onValueChange={(value) => setSalesPersonFilter(String(value))} style={styles.assignPicker}><Picker.Item label={coordinatorFilter ? "All Sales Persons" : "Select coordinator first"} value="" />{salesPeople.map((name) => <Picker.Item key={name} label={name} value={name} />)}</Picker></View></View><View style={styles.assignedLeadGrid}>{visible.length ? visible.map((lead) => <View key={lead.leadId ?? lead.leadCode} style={[styles.assignedLeadCard, [styles.leadToneBlue, styles.leadToneTeal, styles.leadToneViolet, styles.leadToneAmber][leadCardTone(lead)]]}><View style={styles.assignedLeadTop}><View style={styles.assignedLeadCopy}><Text numberOfLines={1} style={styles.assignedLeadName}>{show(lead.clientName)}</Text><Text style={styles.assignedLeadMeta}>{show(lead.mobileNumber)}</Text></View><View style={styles.leadStatusPill}><Text style={styles.leadStatusPillText}>{show(lead.leadStatus).replace(/_/g, ' ')}</Text></View></View><View style={styles.assignedLeadDivider} /><View style={styles.assignedLeadDetails}><View><Text style={styles.assignLabel}>SALES PERSON</Text><Text style={styles.assignedLeadValue}>{show(lead.assignedEmployeeName)}</Text></View><View><Text style={styles.assignLabel}>ASSIGNED DATE</Text><Text style={styles.assignedLeadValue}>{show(lead.assignedAt)}</Text></View><View><Text style={styles.assignLabel}>LAST MEETING / VERIFIED</Text><Text style={styles.assignedLeadValue}>{show(lead.audit?.updatedAt)}</Text></View></View></View>) : <State message="No assigned leads match these filters." />}</View></View>; }
const styles = StyleSheet.create({
  headerSearch: { width: 190, height: 32, paddingHorizontal: 10, borderWidth: 1, borderColor: '#91A7E0', borderRadius: 7, color: '#FFFFFF', fontSize: 9, fontWeight: '700', backgroundColor: 'rgba(255,255,255,0.12)' },
  headerLeadFilters: { flexDirection: 'row', gap: 8 }, headerLeadFilter: { height: 36, flexDirection: 'row', alignItems: 'center', gap: 6 }, headerLeadPicker: { width: 190, height: 36, overflow: 'hidden', borderRadius: 8, backgroundColor: '#FFFFFF' }, headerLeadPickerDisabled: { opacity: 0.5 }, headerLeadPickerControl: { height: 36, color: '#1E3A8A', fontSize: 9, fontWeight: '800' },
