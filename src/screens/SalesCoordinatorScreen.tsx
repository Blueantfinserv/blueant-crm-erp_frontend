...
type AssignedDateFilter = string;
...
export function SalesCoordinatorScreen({ permissions }: { permissions?: readonly string[] | null }) {
  ...
  const [assignedDateFilter, setAssignedDateFilter] = useState<AssignedDateFilter>('');
    ...
    try {
      const rows = (await Promise.all(visibleAssignedLeads.map(async (lead) => {
        ...
        return meetingRows.map((meeting) => [
          detailedLead.clientName, detailedLead.mobileNumber, detailedLead.speciality, detailedLead.location, detailedLead.clinicAddress, detailedLead.leadCode,
          detailedLead.assignedEmployeeName, detailedLead.assignedEmployeeCode, detailedLead.assignedByEmployeeName, detailedLead.assignedByEmployeeCode, detailedLead.assignedAt, detailedLead.leadStatus?.replace(/_/g, ' '), detailedLead.remarks,
          ...
          meeting?.meetingCode, meeting?.meetingTitle ?? meeting?.meetingType, meeting?.meetingStatus, meeting?.meetingDate, timeText(meeting?.meetingTime), meeting?.meetingMode, meeting?.leadStatus?.replace(/_/g, ' '), meeting?.aloneWith, meeting?.personName, meeting?.position, meeting?.remarks ?? meeting?.meetingRemarks ?? meeting?.discussion, meeting?.nextMeetingDate, meeting?.meetingLocation ?? meeting?.location ?? meeting?.address, meeting?.latitude, meeting?.longitude, meeting?.locationAccuracy, meeting?.verifiedBy, meeting?.meetingVerificationDate, meeting?.verificationRemarks,
        ]);
      }))).flat();
      const headers = ['Client Name', 'Mobile Number', 'Speciality', 'Location', 'Clinic Address', 'Lead Code', 'Sales Person', 'Sales Person ID', 'Sales Coordinator', 'SC ID', 'Assigned Date', 'Current Lead Status', 'Lead Remarks', 'SC Age Group', 'SC Prior Investment', 'SC Profession', 'SC Firm / Clinic', 'SC Best Time', 'Meeting Code', 'Meeting Type', 'Meeting Status', 'Meeting Date', 'Meeting Time', 'Meeting Mode', 'Meeting Lead Status', 'Joined With', 'Person Name', 'Position', 'Meeting Remarks', 'Next Plan Date', 'Meeting Location', 'Latitude', 'Longitude', 'Location Accuracy', 'Verified By', 'Verification Date', 'Verification Remarks'];
      const blob = new Blob([`\uFEFF${[headers, ...rows].map((row) => row.map(csvCell).join(',')).join('\r\n')}`], { type: 'text/csv;charset=utf-8' });
      const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'assigned-leads-history.csv'; link.click(); URL.revokeObjectURL(url);
      ...
      <View style={styles.contentHeading}>
        <View style={[styles.contentHeadingRow, compact && styles.contentHeadingRowCompact]}>
          <View>
            ...
            <Text style={styles.contentSubtitle}>{tab === 'today' ? `Showing ${visibleTodayMeetings.length} of ${pending.length} meetings` : tab === 'responses' ? `Showing ${visibleVerifiedMeetings.length} of ${verified.length} verified meetings` : tab === 'tasks' ? `Showing ${filteredTasks.length} of ${taskMeetings.length} active tasks` : tab === 'assignedLeads' ? `Showing ${visibleAssignedLeads.length} of ${assignedLeads.length} assigned leads` : 'Create a physical lead and assign it to a sales person'}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  assignedHeaderActions: { flexDirection: 'row', alignItems: 'center', gap: 8 }, assignedHeaderActionsCompact: { width: '100%', flexDirection: 'column', alignItems: 'stretch' }, headerLeadFilters: { flexDirection: 'row', gap: 8 }, headerLeadFiltersCompact: { width: '100%', flexDirection: 'column', gap: 7 }, headerLeadFilter: { height: 36, flexDirection: 'row', alignItems: 'center', gap: 6 }, headerLeadFilterCompact: { width: '100%' }, headerLeadPicker: { width: 150, height: 36, overflow: 'hidden', borderRadius: 8, backgroundColor: '#FFFFFF' }, headerLeadPickerCompact: { flex: 1, width: undefined }, headerLeadPickerDisabled: { opacity: 0.5 }, headerLeadPickerControl: { height: 36, color: '#1E3A8A', fontSize: 9, fontWeight: '800' }, headerLeadDatePicker: { width: 190, height: 36, flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, borderWidth: 1, borderColor: '#BFD1FF', borderRadius: 8, backgroundColor: '#FFFFFF' }, headerLeadDateText: { minWidth: 0, flex: 1, color: '#1E3A8A', fontSize: 10, fontWeight: '800' },
