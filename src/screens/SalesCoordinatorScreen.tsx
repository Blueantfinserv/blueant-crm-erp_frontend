...
function LeadHistoryContentCompact({ lead, meetings, loading, error }: { lead: LeadResponse; meetings: MeetingResponse[]; loading: boolean; error: string | null }) {
  const verifiedMeeting = meetings.find((meeting) => meeting.verificationStatus === 'VERIFIED' || Boolean(meeting.verifiedBy));
  const latestRemark = meetings.find((meeting) => ['COMPLETED', 'VERIFIED'].includes(String(meeting.meetingStatus ?? meeting.verificationStatus ?? '').toUpperCase()) && (meeting.remarks || meeting.meetingRemarks || meeting.discussion));
...
function MeetingHistoryRow({ meeting, alternate }: { meeting: MeetingResponse; alternate: boolean }) { const status = String(meeting.meetingStatus ?? meeting.leadStatus ?? 'PENDING').replace(/_/g, ' '); const normalized = status.toUpperCase(); const statusStyle = normalized.includes('COMPLETED') ? historyStyles.statusCompleted : normalized.includes('SCHEDULED') ? historyStyles.statusScheduled : historyStyles.statusPending; const completed = normalized.includes('COMPLETED') || normalized.includes('VERIFIED'); const remark = completed ? meeting.remarks ?? meeting.meetingRemarks ?? meeting.discussion : null; return <View style={[historyStyles.meetingRow, alternate && historyStyles.meetingRowAlt]}><View style={historyStyles.meetingTitleBlock}><Text style={historyStyles.meetingTitle}>{show(meeting.meetingTitle ?? meeting.meetingType)}</Text><Text style={historyStyles.meetingDate}>{show(meeting.meetingDate)}</Text></View>{meeting.meetingTiming ? <Text style={[historyStyles.meetingField, historyStyles.timePill]}>Time: {meeting.meetingTiming}</Text> : null}{meeting.meetingWith ? <Text style={[historyStyles.meetingField, historyStyles.joinPill]}>Joined: {meeting.meetingWith.replace(/_/g, ' ')}</Text> : null}{meeting.personName ? <Text style={[historyStyles.meetingField, historyStyles.personPill]}>Person: {meeting.personName}</Text> : null}{meeting.position ? <Text style={[historyStyles.meetingField, historyStyles.positionPill]}>Position: {meeting.position}</Text> : null}<Text style={[historyStyles.meetingStatus, statusStyle]}>{status}</Text>{remark ? <Text numberOfLines={1} style={historyStyles.meetingRemark}>{remark}</Text> : null}</View>; }
...
const historyStyles = StyleSheet.create({
  ...
  meetingTitleBlock: { minWidth: 120 },
  ...
  meetingStatus: { marginLeft: 'auto', paddingHorizontal: 7, paddingVertical: 3, borderRadius: 99, fontSize: 7, fontWeight: '900' },
